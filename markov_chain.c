...
Node* add_to_database(MarkovChain* markov_chain, char* data_ptr)
{
    ...
    new_node->data->data = strdup(data_ptr);
...
int add_node_to_frequency_list(MarkovNode* first_node, MarkovNode* second_node)
{
    ...
    {
        frequency_list = malloc(2 * sizeof(MarkovNodeFrequency));
        if (frequency_list == NULL)
        {
    ...
    MarkovNodeFrequency* new_list = realloc(frequency_list,
                                            (i + 2) * sizeof(MarkovNodeFrequency));
    if (new_list == NULL)
    {
        printf("Memory allocation failed in add_node_to_frequency_list()\n");
        return EXIT_FAILURE;
...
void free_database(MarkovChain** ptr_chain) {
    ...
    while (current_node != NULL) {
        ...
        if (markov_node != NULL) {
            ...
            // Free the string data in the MarkovNode
...
MarkovNode* get_first_random_node(MarkovChain *markov_chain){
  ...
  const int total_length = get_chain_length(markov_chain->database);
  if (total_length == 0){
    return NULL;
  }
...
MarkovNode* get_next_random_node(MarkovNode *cur_markov_node){
  if (cur_markov_node == NULL || cur_markov_node->frequency_list == NULL){
    ...
    MarkovNodeFrequency* frequency_list = cur_markov_node->frequency_list;
