...
typedef struct MarkovChain{
    LinkedList *database;
    /*a pointer to a linked list of nodes, each containing MarkovNode that
    corresponds to a unique word in the text.
     */
} MarkovChain;

/*
this struct describes the vertices of the Markov Chain
 */
typedef struct MarkovNode{
    char *data;
    //points to the content of the word
    struct MarkovNodeFrequency *frequency_list;
    /*a pointer used as a dynamic array of MarkovnodeFrequency elements
      this array contains all the possible subsequent words after the current
    word, by using the given text.
    THE FREEING AND CREATION OF THIS ARRAY IS MY RESPONSIBILITY
    for finishing words, this field points to NULL
     */
